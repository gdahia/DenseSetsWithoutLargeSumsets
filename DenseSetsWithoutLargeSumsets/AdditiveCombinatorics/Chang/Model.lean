...
import DenseSetsWithoutLargeSumsets.AdditiveCombinatorics.Chang.FreimanDimension
...
lemma rationalVectorToReal_ratCoordHom_eq_cast {r} (v : Fin r → ℤ) :
    rationalVectorToReal (ratCoordHom v) = fun i ↦ (v i : ℝ) := by
  ext i
  dsimp only [ratCoordHom, rationalVectorToReal]
  exact_mod_cast rfl
...
/-! ## Effective models of arbitrary integral Freiman coordinates -/

variable {G : Type*} [AddCommGroup G]

open Classical in
/-- The translated image of a finite set under an integral coordinate map. -/
@[reducible]
def translatedImageCoords (f : G → Fin d → ℤ) (X : Finset G) (x₀ : G) :
    Finset (Fin d → ℤ) :=
  X.image (fun x ↦ f x - f x₀)

/-- The effective dimension of an integral coordinate map on a finite set. -/
@[reducible]
def effectiveModelDim (f : G → Fin d → ℤ) (X : Finset G) (x₀ : G) : ℕ :=
  Module.finrank ℝ (integerGeneratedSubspace (translatedImageCoords f X x₀))

/-- The full-rank rational model associated with an integral coordinate map. -/
@[reducible]
noncomputable def effectiveModelFn (f : G → Fin d → ℤ) (X : Finset G) (x₀ : G) :
    G → Fin (effectiveModelDim f X x₀) → ℚ :=
  fun x ↦ latticeModelMap (translatedImageCoords f X x₀) (f x - f x₀)

/-- A Freiman coordinate map remains a Freiman isomorphism after translation and passage to
effective saturated-lattice coordinates. -/
lemma effectiveModelFn_isAddFreimanIso (f : G → Fin d → ℤ) (X : Finset G)
    (hf : IsAddFreimanIso 2 (X : Set G) (f '' (X : Set G)) f) (x₀ : G) :
    IsAddFreimanIso 2 (X : Set G)
      (effectiveModelFn f X x₀ '' (X : Set G)) (effectiveModelFn f X x₀) := by
  have htrans : IsAddFreimanIso 2 (f '' (X : Set G))
      ((fun y ↦ y - f x₀) '' (f '' (X : Set G))) (fun y ↦ y - f x₀) :=
    isAddFreimanIso_sub_const _ _
  have hcoords : IsAddFreimanIso 2 (X : Set G)
      ((fun y ↦ y - f x₀) '' (f '' (X : Set G)))
      ((fun y ↦ y - f x₀) ∘ f) :=
    htrans.comp hf
  have hlattice :
      IsAddFreimanIso 2 (translatedImageCoords f X x₀ : Set (Fin d → ℤ))
        (latticeModelMap (translatedImageCoords f X x₀) ''
          (translatedImageCoords f X x₀ : Set (Fin d → ℤ)))
        (latticeModelMap (translatedImageCoords f X x₀)) :=
    latticeModelMap_isAddFreimanIso _
  have himage :
      ((fun y ↦ y - f x₀) '' (f '' (X : Set G))) =
        (translatedImageCoords f X x₀ : Set (Fin d → ℤ)) := by
    simp only [Set.image_image, translatedImageCoords, Finset.coe_image]
  rw [himage] at hcoords
  have hcomp := hlattice.comp hcoords
  have htarget :
      latticeModelMap (translatedImageCoords f X x₀) ''
          (translatedImageCoords f X x₀ : Set (Fin d → ℤ)) =
        effectiveModelFn f X x₀ '' (X : Set G) := by
    simp only [translatedImageCoords, effectiveModelFn, Finset.coe_image, Set.image_image]
  rw [← htarget]
  exact hcomp

omit [AddCommGroup G] in
lemma effectiveModelFn_mem_contains_zero (f : G → Fin d → ℤ) (X : Finset G)
    (x₀ : G) (hx₀ : x₀ ∈ X) :
    (0 : Fin (effectiveModelDim f X x₀) → ℝ) ∈
      ((X.image (effectiveModelFn f X x₀)).image rationalVectorToReal :
        Set (Fin (effectiveModelDim f X x₀) → ℝ)) := by
  refine Finset.mem_image.mpr ⟨effectiveModelFn f X x₀ x₀, ?_, ?_⟩
  · exact Finset.mem_image.mpr ⟨x₀, hx₀, rfl⟩
  · change rationalVectorToReal
      (latticeModelMap (translatedImageCoords f X x₀) (f x₀ - f x₀)) = 0
    rw [sub_self, latticeModelMap_zero, rationalVectorToReal_zero]

omit [AddCommGroup G] in
lemma latticeModelMap_effectiveModelFn_rfl (f : G → Fin d → ℤ)
    (X : Finset G) (x₀ y : G) :
    latticeModelMap (translatedImageCoords f X x₀) (f y - f x₀) =
      effectiveModelFn f X x₀ y := rfl

omit [AddCommGroup G] in
/-- The effective model has full affine dimension. -/
lemma effectiveModelFn_affineDim (f : G → Fin d → ℤ) (X : Finset G)
    (x₀ : G) (hx₀ : x₀ ∈ X) :
    finsetAffineDim ((X.image (effectiveModelFn f X x₀)).image rationalVectorToReal) =
      effectiveModelDim f X x₀ := by
  let Y := translatedImageCoords f X x₀
  let r := effectiveModelDim f X x₀
  let H : Submodule ℝ (Fin d → ℝ) := integerGeneratedSubspace Y
  let e : H ≃ₗ[ℝ] Fin r → ℝ := integerGeneratedCoordinateEquiv Y
  set T : Finset (Fin r → ℝ) :=
    (X.image (effectiveModelFn f X x₀)).image rationalVectorToReal with hTdef
  have h0 : (0 : Fin r → ℝ) ∈ (T : Set (Fin r → ℝ)) :=
    effectiveModelFn_mem_contains_zero f X x₀ hx₀
  set Z : Set H := {p : H | ∃ y ∈ (Y : Set (Fin d → ℤ)), ↑p = intVectorToReal y}
  have hT_eq : (T : Set (Fin r → ℝ)) = e '' Z := by
    ext x
    constructor
    · intro hx
      rw [hTdef, Finset.coe_image, Finset.coe_image, Set.mem_image] at hx
      obtain ⟨w, hw_mem, rfl⟩ := hx
      rw [Set.mem_image] at hw_mem
      obtain ⟨y, hyX, rfl⟩ := hw_mem
      set yvec : Fin d → ℤ := f y - f x₀
      have hyY : yvec ∈ ↑Y := Finset.mem_image.mpr ⟨y, hyX, rfl⟩
      refine ⟨⟨intVectorToReal yvec, mem_latticeDomain_of_mem Y hyY⟩,
        ⟨yvec, hyY, rfl⟩, ?_⟩
      rw [← latticeModelMap_effectiveModelFn_rfl f X x₀ y,
        rationalVectorToReal_latticeModelMap Y (mem_latticeDomain_of_mem Y hyY)]
      rfl
    · rintro ⟨p, hpZ, rfl⟩
      obtain ⟨yvec, hyY, hp⟩ := hpZ
      obtain ⟨y, hyX, hyeq⟩ := Finset.mem_image.mp hyY
      rw [hTdef, Finset.coe_image, Finset.coe_image, Set.mem_image]
      refine ⟨effectiveModelFn f X x₀ y, ⟨y, hyX, rfl⟩, ?_⟩
      rw [← latticeModelMap_effectiveModelFn_rfl f X x₀ y, hyeq,
        rationalVectorToReal_latticeModelMap Y (mem_latticeDomain_of_mem Y hyY)]
      congr 1
      apply Subtype.ext
      exact hp.symm
  have hsubtype_image : H.subtype '' Z = intVectorToReal '' (Y : Set (Fin d → ℤ)) := by
    ext x
    constructor
    · rintro ⟨p, hp, rfl⟩
      obtain ⟨y, hyY, hpy⟩ := hp
  ...
  have hvspan : vectorSpan ℝ (T : Set (Fin r → ℝ)) = ⊤ := by
    ...
    have hsub2 : (T : Set (Fin r → ℝ)) ⊆
      ...
      rw [vsub_eq_sub, sub_zero]
    have heq : Submodule.span ℝ
        ((· -ᵥ (0 : Fin r → ℝ)) '' (T : Set (Fin r → ℝ))) =
        Submodule.span ℝ (T : Set (Fin r → ℝ)) :=
      le_antisymm (Submodule.span_mono hsub1) (Submodule.span_mono hsub2)
    rw [heq, hspanT]
  change Module.finrank ℝ (affineSpan ℝ (T : Set (Fin r → ℝ))).direction = r
  rw [direction_affineSpan, hvspan]
  exact (finrank_top ℝ (Fin r → ℝ)).trans (Module.finrank_fin_fun ℝ)

/-- The effective rank of any integral Freiman coordinate model is at most the input's Freiman
dimension. -/
theorem effectiveModelDim_le_freimanDim [DecidableEq G]
    (f : G → Fin d → ℤ) (X : Finset G)
    (hf : IsAddFreimanIso 2 (X : Set G) (f '' (X : Set G)) f)
    {x₀ : G} (hx₀ : x₀ ∈ X) :
    effectiveModelDim f X x₀ ≤ freimanDim X := by
  have hmodel : freimanModelDim X (effectiveModelDim f X x₀) := by
    refine ⟨effectiveModelFn f X x₀, ?_, effectiveModelFn_affineDim f X x₀ hx₀⟩
    have hiso := effectiveModelFn_isAddFreimanIso f X hf x₀
    rw [Finset.coe_image]
    exact hiso
  have hcard : effectiveModelDim f X x₀ ≤ X.card := by
    have h := finsetAffineDim_le_card
      ((X.image (effectiveModelFn f X x₀)).image rationalVectorToReal)
    rw [effectiveModelFn_affineDim f X x₀ hx₀] at h
    exact h.trans
      ((Finset.card_image_le (f := rationalVectorToReal)).trans
        (Finset.card_image_le (f := effectiveModelFn f X x₀)))
