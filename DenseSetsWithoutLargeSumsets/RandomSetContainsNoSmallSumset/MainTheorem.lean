...
theorem small_sumset_pair_probability_le_fingerprint_sum {γ C c : ℝ} {n : ℕ}
  ...
  · change (smallSumsetMeasure n δ).real
      {S : Finset ℕ |
        pairSumsetIsSubset n (pairCardThreshold (2 + γ) n δ) 0
          (C * pairCardThreshold (2 + γ) n δ) S} ≤ _
...
private lemma lower_dim_fingerprint_sum_bound {γ C c : ℝ} {n : ℕ} {δ : unitInterval}
    ...
    (hδ_upper_c : (δ : ℝ) ≤ 1 - c) :
      ∑ P ∈ lowerModelGAPs (γ := γ) (C := C) (n := n) δ hγ_pos C_pos hn,
        ∑ p ∈ bltDimSmallWitnessPairs
            ...
            P.dim (pairCardThreshold (2 + γ) n δ) C γ,
          (δ : ℝ) ^ (p.1 + p.2).card ≤
      2 * (pairCardThreshold (2 + γ) n δ : ℝ) / (n : ℝ) ^ (ε γ / 2) := by
  classical
  ...
  have hkpos : 0 < pairCardThreshold (2 + γ) n δ :=
    pairCardThreshold_pos_of_lower_density hγ_pos hn hδ_lower hδ_upper
  apply le_trans (b :=
    ∑ d ∈ Finset.Icc 1 (pairCardThreshold (2 + γ) n δ),
  ...
  apply le_trans (b := ∑ d ∈ Finset.Icc 1 (pairCardThreshold (2 + γ) n δ),
    (n : ℝ) ^ (-(ε γ / 2)))
  ...
  · rw [Finset.sum_const, nsmul_eq_mul]
    have hcoef : ((Finset.Icc 1 (pairCardThreshold (2 + γ) n δ)).card : ℝ) ≤
        2 * (pairCardThreshold (2 + γ) n δ : ℝ) := by
      have hcard : ((Finset.Icc 1 (pairCardThreshold (2 + γ) n δ)).card : ℝ) ≤
          (pairCardThreshold (2 + γ) n δ : ℝ) := by
        rw [Nat.card_Icc]
        exact_mod_cast (Nat.succ_sub_one
          (pairCardThreshold (2 + γ) n δ)).le
      nlinarith [hcard, ((by exact_mod_cast hkpos) :
        (0 : ℝ) < pairCardThreshold (2 + γ) n δ)]
...
theorem small_sumset_pair_probability_le {γ C c : ℝ} {n : ℕ} {δ : unitInterval}
  ...
  · change (smallSumsetMeasure n δ).real
        ...
        2 * (pairCardThreshold (2 + γ) n δ : ℝ) / (n : ℝ) ^ (ε γ / 2)
    rw [(Set.eq_empty_iff_forall_notMem (s := {S : Finset ℕ |
        pairSumsetIsSubset n (pairCardThreshold (2 + γ) n δ) 0
          (C * pairCardThreshold (2 + γ) n δ) S})).mpr (by
      intro S hS
